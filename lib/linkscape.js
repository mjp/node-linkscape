...
(function() {
    ...
    this.send = function(apiPath, params, method, callback) {
		...
		/** Full path with the hostname was causing it to fail with a 
		 * status: 403, error: Unrecognized API method: 'No api method found.'
		 * Note that parameter names are case-sensitive. For more information on available calls, 
		 * see: http://apiwiki.moz.com/categories/api-reference"
		 * 
		 * Original code: var path = 'http://' + this.options.hostname + '/' + this.options.path + '/' + apiPath;
		 * Modified it to include only the path, leaving out the hostname
		 */
        var args = [];
