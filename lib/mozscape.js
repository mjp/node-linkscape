...
(function() {
    ...
    this.bulkUrlMetrics = function(urlsArray, cols, callback) {
        var apiPath = 'url-metrics/';

        var cols = this._translateBitfield(cols, this.URL_METRICS_FLAGS);
        apiPath += '?Cols=' + cols;

        var expires = Date.now() + 300;
        var stringToSign = this.options.accessId + "\n" + expires;
        var binarySignature = crypto.createHmac('sha1', this.options.secret).update(stringToSign).digest('base64');
        var urlSafeSignature = encodeURIComponent(binarySignature);
        ...
        var params = JSON.stringify(urlsArray);
        var options = {
            url: path,
            method: 'POST',
            headers: {
